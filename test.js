...
const anzahlFehlerProBuchstabe = {
    a: ["a", 55, 0], b: ["b", 1, 0], c: ["c", 0, 0], d: ["d", 5, 0], e: ["e", 0, 0], f: ["f", 0, 0],
...
debugger
const copyObjekt = anzahlFehlerProBuchstabe
function test(anzahlFehlerProBuchstabe) {
    let platz1 = ["", 0, 0]
    let platz2 = ["", 0, 0]
    let plätze = [platz1, platz2]
    console.log(platz1 === platz2)
    //a: ["a", 55, 0]
    for (const buchstabe in anzahlFehlerProBuchstabe) {
        let akBuchstabe = anzahlFehlerProBuchstabe[buchstabe][1] // 0
        for (let i = 0; i < plätze.length; i++) {
            if (akBuchstabe >= plätze[i][1]) {
                plätze[i] = anzahlFehlerProBuchstabe[buchstabe]
            }
        }
    }
    ...
    // for (const buchstabe in anzahlFehlerProBuchstabe) {
    //     let akBuchstabe = anzahlFehlerProBuchstabe[buchstabe][1]
    //     if (akBuchstabe >= platz2[1] && akBuchstabe <= platz1[1] && anzahlFehlerProBuchstabe[buchstabe][0] !== platz1[0]) {

    //             platz2 = anzahlFehlerProBuchstabe[buchstabe]
    //     }
    // }
    return [platz1, platz2]
}

function sagJaoderNein(arrey) {

}


console.log(0 + NaN, "0" + NaN, 0 + Infinity, 1 + false, "" + true)
console.log(test(copyObjekt))
